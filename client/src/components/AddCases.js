...
const AddCases = ({ isOpen, onClose, onSubmit, contract ,account }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (currEvidenceFiles || currCaseData['key1'].length > 0 || currCaseData['key2'].length > 0 || currCaseData['key3'].length > 0) {
            try {
                ...
                if (currCaseData['key1'].length > 0 || currCaseData['key2'].length > 0 || currCaseData['key3'].length > 0) {
                    ...
                    const file = new File([blob], `${currentTimestampSeconds}.json`, { type: 'application/json' });
        ...
        const resFromContractAddCase = await contract.addCase(desc,currentTimestampSeconds,Cids,collected_by_list);
                ...
                <form className='AddCaseForm' onSubmit={handleSubmit}>
                    <div className="sections">
                        <div className="leftSide">
                            ...
                            <input type="text" name="collected_by" id="collected_by" placeholder='Collected By' onChange={handleChange} />

                            <input type="file" name="evidenceFiles" id="evidenceFiles" className="evidenceFiles" multiple onChange={handleFileChange} />
                            <label htmlFor="evidenceFiles" className="evidenceFilesLabel" id='fileLabel'>Add Evidences Images</label>
                            {
                        ...
                        <div className="rightSide">
                            ...
                            <div className="extraInfo">
                                <input className="key key1" name='key1' id='key1' placeholder='key1' onChange={handleChange}></input>
                                <input className="value value1" name='value1' id='value1' placeholder='value1' onChange={handleChange}></input>
                            </div>
                            <div className="extraInfo">
                                <input className="key key2" name='key2' id='key2' placeholder='key2' onChange={handleChange}></input>
                                <input className="value value2" name='value2' id='value2' placeholder='value2' onChange={handleChange}></input>
                            </div>
                            <div className="extraInfo">
                                <input className="key key3" name='key3' id='key3' placeholder='key3' onChange={handleChange}></input>
                                <input className="value value3" name='value3' id='value3' placeholder='value3' onChange={handleChange}></input>
                            </div>
                        </div>
                    </div>
                    <div className='btnDiv'>
                        <button className="AddCaseBtn bold" type='submit'>Add Case</button>
                    </div>
                </form>
            </div>
        </div>
